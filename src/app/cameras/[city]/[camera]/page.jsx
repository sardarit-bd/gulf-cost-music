"use client";
import { ArrowLeft, Camera, Heart, Mail, MapPin, Share2, Video } from "lucide-react";
...
export default function PhotographerProfile() {
    ...
    useEffect(() => {
        fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/photographers/${photographerId}`)
            .then((res) => res.json())
            .then((data) => {
                console.log(data)
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-gray-900 to-black flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-yellow-400 text-lg">Loading photographer profile...</p>
                </div>
            </div>
        );
    }
                ...
                <div className="relative container mx-auto h-full flex items-end pb-6 sm:pb-8 px-4 sm:px-6">
                    ...
                    <div className="absolute top-4 sm:top-6 left-4 sm:left-6 right-4 sm:right-6 flex items-center justify-between">
                        ...
                        <div className="flex gap-2 sm:gap-3">
                            ...
                            >
                                <Heart size={18} className="sm:w-5" fill={isFavorite ? "currentColor" : "none"} />
                            </button>
                        ...
                        <div className="flex-1 pb-1 sm:pb-2 min-w-0">
                            ...
                            <div className="flex flex-col xs:flex-row xs:items-center gap-2 sm:gap-4 text-white/80 text-sm sm:text-base">
                                <div className="flex items-center gap-2 bg-white/10 backdrop-blur-sm px-2 sm:px-3 py-1 rounded-full w-fit">
                                    ...
                                    <span className="capitalize font-medium">{photographer.city}</span>
                                </div>
                    ...
                    <div className="lg:col-span-1 space-y-4 sm:space-y-6">
                        ...
                        <div className="bg-gray-800/60 backdrop-blur-lg rounded-xl sm:rounded-2xl shadow-2xl p-4 sm:p-6 border border-gray-700/50 lg:sticky lg:top-24">
                            ...
                            <div className="flex items-center gap-3 p-3 sm:p-4 bg-gray-700/50 text-white rounded-lg sm:rounded-xl border border-gray-600/50 text-sm sm:text-base">
                                ...
                                <span className="font-medium truncate">{photographer.user?.email}</span>
                            </div>
                        </div>
                        ...
                        <div className="bg-gray-800/60 backdrop-blur-lg rounded-xl sm:rounded-2xl shadow-2xl p-4 sm:p-6 border border-gray-700/50">
                            ...
                            <div className="grid grid-cols-2 gap-3 sm:gap-4">
                                <div className="text-center p-3 sm:p-4 bg-gray-700/30 rounded-lg sm:rounded-xl border border-gray-600/30">
                                    <div className="text-xl sm:text-2xl font-bold text-yellow-400">{photographer.services?.length || 0}</div>
                                    <div className="text-xs sm:text-sm text-gray-300 mt-1">Services</div>
                                </div>
                                <div className="text-center p-3 sm:p-4 bg-gray-700/30 rounded-lg sm:rounded-xl border border-gray-600/30">
                                    <div className="text-xl sm:text-2xl font-bold text-green-400">{photographer.photos?.length || 0}</div>
                                    <div className="text-xs sm:text-sm text-gray-300 mt-1">Photos</div>
                                </div>
                                <div className="text-center p-3 sm:p-4 bg-gray-700/30 rounded-lg sm:rounded-xl border border-gray-600/30">
                                    <div className="text-xl sm:text-2xl font-bold text-purple-400">{photographer.videos?.length || 0}</div>
                                    <div className="text-xs sm:text-sm text-gray-300 mt-1">Videos</div>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="bg-gray-800/60 backdrop-blur-lg rounded-xl sm:rounded-2xl shadow-2xl mb-6 sm:mb-8 border border-gray-700/50 overflow-hidden">
                            <div className="border-b border-gray-700/50">
                                <div className="flex overflow-x-auto scrollbar-hide">
                                    {[
                                        { id: "portfolio", label: "Portfolio", icon: Camera, count: photographer.photos?.length },
                                        { id: "about", label: "About", icon: MapPin },
                                        { id: "videos", label: "Videos", icon: Video, count: photographer.videos?.length },
                                    ].map((tab) => (
                                        <button
                                            key={tab.id}
                                        ...
                                        >
                                            ...
                                            {tab.label}
                                            {tab.count > 0 && (
                                                <span className={`px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full text-xs font-bold ${activeTab === tab.id
                            ...
                            <div className="p-4 sm:p-6 md:p-8">
                                {activeTab === "portfolio" && (
                                    <div>
                                        <h3 className="text-xl sm:text-2xl font-bold text-white mb-2">Portfolio Gallery</h3>
                                        <p className="text-gray-400 mb-4 sm:mb-6 text-sm sm:text-base">Explore {photographer.name}'s stunning photography work</p>
                                        ...
                                        {photographer.photos && photographer.photos.length > 0 ? (
                                                ...
                                                {photographer.photos.map((photo, index) => (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-3 sm:p-4">
                                                            <span className="text-white font-semibold text-sm sm:text-base">Photo {index + 1}</span>
                                                        </div>
                                            ...
                                            <div className="text-center py-12 sm:py-16">
                                                ...
                                                <p className="text-gray-400 text-base sm:text-lg">No portfolio photos available yet.</p>
                                            </div>
                                        )}
                                    </div>
                                )}
                                ...
                                {activeTab === "about" && (
                                    <div>
                                        <h3 className="text-xl sm:text-2xl font-bold text-white mb-2">About {photographer.name}</h3>
                                        <p className="text-gray-400 mb-4 sm:mb-6 text-sm sm:text-base">Get to know the artist behind the lens</p>
                                        ...
                                        <div className="prose prose-invert max-w-none">
                                            <p className="text-gray-300 leading-relaxed text-base sm:text-lg bg-gray-700/30 rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-600/30">
                                                {photographer.biography || "No biography available yet. This photographer prefers to let their work speak for itself."}
                                            </p>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mt-6 sm:mt-8">
                                            <div className="bg-gradient-to-br from-gray-700/50 to-gray-800/50 rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-600/30">
                                                <h4 className="font-semibold text-white mb-3 sm:mb-4 flex items-center gap-2 sm:gap-3 text-sm sm:text-base">
                                                    <MapPin className="text-yellow-400" size={20} className="sm:w-6" />
                                                    Location & Coverage
                                                </h4>
                                                <div className="space-y-2 sm:space-y-3">
                                                    <div className="flex items-center justify-between p-2 sm:p-3 bg-gray-600/30 rounded-lg text-sm sm:text-base">
                                                        ...
                                                        <span className="text-white font-semibold capitalize">{photographer.city}</span>
                                                    </div>
                                                    <div className="flex items-center justify-between p-2 sm:p-3 bg-gray-600/30 rounded-lg text-sm sm:text-base">
                                                        <span className="text-gray-300">Coverage Area</span>
                                                        <span className="text-white font-semibold">Gulf Coast</span>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="bg-gradient-to-br from-gray-700/50 to-gray-800/50 rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-600/30">
                                                <h4 className="font-semibold text-white mb-3 sm:mb-4 flex items-center gap-2 sm:gap-3 text-sm sm:text-base">
                                                    <Camera className="text-green-400" size={20} className="sm:w-6" />
                                                    Specialties
                                                </h4>
                                ...
                                {activeTab === "videos" && (
                                    <div>
                                        <h3 className="text-xl sm:text-2xl font-bold text-white mb-2">Video Portfolio</h3>
                                        <p className="text-gray-400 mb-4 sm:mb-6 text-sm sm:text-base">Motion and storytelling through video</p>
                                        ...
                                        {photographer.videos && photographer.videos.length > 0 ? (
                                                ...
                                                {photographer.videos.map((video, index) => (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex items-center justify-between">
                                                            <h4 className="font-semibold text-white text-base sm:text-lg truncate mr-2">{video.title}</h4>
                                                            <span className="text-gray-400 text-xs sm:text-sm whitespace-nowrap">Video {index + 1}</span>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                                            ...
                                            <div className="text-center py-12 sm:py-16">
                                                ...
                                                <p className="text-gray-400 text-base sm:text-lg">No videos available yet.</p>
                                            </div>
                                        )}
                                    </div>
                                )}
