...
import StatCard from "@/components/modules/dashboard/studio/StatCard";
...
export default function StudioDashboard() {
    ...
    const calculateProfileCompletion = (data) => {
        let score = 0;
        const fields = [
            data?.name, data?.city, data?.state, data?.biography,
            data?.services?.length > 0, data?.photos?.length > 0,
            data?.audioFile?.url
        ];
    ...
    const quickActions = [
        ...
        },
        {
            ...
            color: "bg-purple-500",
            badge: studioData && calculateProfileCompletion(studioData) < 100 ? "Required" : null
        },
        {
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-3xl md:text-4xl font-bold text-gray-900">
                            Welcome back, <span className="text-blue-600">{studioData?.name || "Studio"}</span>!
                        </h1>
                    ...
                    <div className="flex items-center gap-4">
                        <div className={`px-4 py-2 rounded-full ${studioData?.isVerified ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'} font-medium flex items-center gap-2`}>
                            {studioData?.isVerified ? <CheckCircle className="w-4 h-4" /> : <AlertCircle className="w-4 h-4" />}
                            {studioData?.isVerified ? 'Verified Studio' : 'Verification Pending'}
                        </div>
                        ...
                        <div key={index} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
                            <div className="flex items-center justify-between">
                                ...
                                <div className={`w-12 h-12 ${stat.color} rounded-xl flex items-center justify-center`}>
                                    {stat.label === "Profile Completion" && <CheckCircle className="w-6 h-6 text-white" />}
                                    {stat.label === "Verified" && <Star className="w-6 h-6 text-white" />}
                                    {stat.label === "Services" && <Package className="w-6 h-6 text-white" />}
                                    {stat.label === "Photos" && <Camera className="w-6 h-6 text-white" />}
                                </div>
                            </div>
                            {stat.label === "Profile Completion" && (
                                <div className="mt-4">
                ...
                <div className="space-y-8">
                    ...
                    <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl p-6 text-white">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <div className="text-3xl font-bold">{calculateProfileCompletion(studioData)}%</div>
                        </div>

                        <div className="space-y-3">
                            {[
                                { label: "Basic Info", completed: !!studioData?.name && !!studioData?.city },
                                { label: "Biography", completed: !!studioData?.biography },
                                { label: "Services", completed: (studioData?.services?.length || 0) > 0 },
                                { label: "Photos", completed: (studioData?.photos?.length || 0) > 0 },
                                { label: "Audio Sample", completed: !!studioData?.audioFile?.url }
                            ].map((item, index) => (
                                <div key={index} className="flex items-center justify-between">
                                    <span className="text-sm">{item.label}</span>
                                    <div className={`w-6 h-6 rounded-full flex items-center justify-center ${item.completed ? 'bg-green-400' : 'bg-gray-300'}`}>
                                        {item.completed ? <CheckCircle className="w-4 h-4 text-white" /> : null}
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
                        ...
                        <ul className="space-y-3">
                            <li className="flex items-start gap-3">
                                ...
                                <span className="text-sm text-gray-600">Upload high-quality studio photos to attract more clients</span>
                            </li>
                            <li className="flex items-start gap-3">
                                <div className="w-6 h-6 bg-green-100 text-green-600 rounded-lg flex items-center justify-center flex-shrink-0">
                                    <Music className="w-3 h-3" />
                                </div>
                                <span className="text-sm text-gray-600">Add audio samples to showcase your production quality</span>
                            </li>
                            <li className="flex items-start gap-3">
                                <div className="w-6 h-6 bg-purple-100 text-purple-600 rounded-lg flex items-center justify-center flex-shrink-0">
                                    <TrendingUp className="w-3 h-3" />
                                </div>
                                <span className="text-sm text-gray-600">Complete profile for 40% more visibility in search results</span>
                            </li>
                        </ul>
                    </div>
                </div>
