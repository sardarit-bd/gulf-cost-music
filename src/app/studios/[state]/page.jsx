...
export default function StudiosByState() {
  ...
  useEffect(() => {
    if (studios.length > 0) {
      const uniqueCities = [
        ...new Set(studios.map((s) => s.city.toLowerCase())),
      ]
        .sort()
        .map((city) => ({
          value: city,
  ...
  const fetchStudiosByState = async () => {
    try {
      setLoading(true);
      // Send state in proper format
      const response = await axios.get(
        `${API_BASE}/api/studios/location?state=${formattedState}`,
      );
      ...
      try {
        const allResponse = await axios.get(`${API_BASE}/api/studios/location`);
        const filtered = allResponse.data.data.filter(
          (s) => s.state.toLowerCase() === formattedState.toLowerCase(),
        );
        setStudios(filtered);
      } catch (fallbackError) {
        ...
        {cities.length > 0 && (
          <div className="mt-12 bg-white/5 rounded-2xl p-8 border border-white/10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {cities.map((city) => {
                const cityStudios = studios.filter(
                  (s) => s.city.toLowerCase() === city.value.toLowerCase(),
                ).length;

                return (
                  <Link
                    key={city.value}
                    href={`/studios/${encodeURIComponent(formattedState)}/${encodeURIComponent(city.value)}`}
