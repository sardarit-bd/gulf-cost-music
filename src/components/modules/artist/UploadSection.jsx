...
export default function UploadSection({
  ...
  const canUpload = currentFiles.length < maxFiles;
  ...
  // ❌ Pro plan restriction UI removed completely
  return (
    <div className="space-y-4">
      ...
      <div
        ...
        onDrop={handleDrop}
        onClick={() =>
          canUpload && document.getElementById(`${type}-upload`).click()
        }
      >
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-5 gap-3">
            {validCurrentFiles.map((file, index) => {
              const showImage =
                type === "image" && file.url && isValidImageUrl(file.url);
                  ...
                  {showImage ? (
                    ...
                    <div
                      className={`w-10 h-10 rounded flex items-center justify-center flex-shrink-0 ${
                        type === "image" ? "bg-gray-700" : "bg-purple-500/20"
                      }`}
                    >
                      {type === "image" ? (
